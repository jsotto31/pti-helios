<template>
  <v-container>
    <PageHeader
      ...
      subtitle="Manage Correction logs for employees."
    >
      <template #create>
      ...
      <v-col cols="12">
        <v-card-text class="py-5">
          <v-container>
            <v-row>
              <v-col cols="6">
                <label for="online-request-type" class="text-subtitle-1"
                  >Status</label
                >
                <select-status
              ...
              <v-row>
                ...
                <v-col cols="12" class="px-1">
                  <v-data-table-server
                    ...
                    :items="correction_application.data"
                    :items-length="correction_application.total"
                    :loading="status == 'pending'"
                    :search="search"
                    item-value="name"
                    v-model:page="page"
                    class="border"
                    :headers="correction_application.headers"
                      ...
                      <div class="d-flex" style="gap: 5px">
                        <EditDialog
                          ...
                          :correction_application="
                            item as CorrectionApplication
                          "
                        >
                        </EditDialog>
                        <DeleteDialog
                          @delete="execute"
                          :id="(item as CorrectionApplication).id"
                        ></DeleteDialog>
                      </div>
                    </template>
                  </v-data-table-server>
...
const {
  data: correction_application,
  status,
  execute,
} = await useApiFetch("/online-application/correction-applications", {
  method: "GET",
  immediate: true,
