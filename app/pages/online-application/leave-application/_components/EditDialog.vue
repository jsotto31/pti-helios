        ...
        <v-dialog width="900" v-model="dialog" persistent>
            ...
            <template #default="{ isActive }">
                <v-card class="pa-5 rounded-lg" :disabled="status == 'pending'">
                    <v-card-title class="pr-4 text-center">Edit Leave Application</v-card-title>
                    <v-divider></v-divider>
                    <v-card-text class="px-4">
                        <v-container>
                            ...
                            <v-row class="mb-0 pb-0">
                                ...
                                <v-col class="pa-0" cols="7">
                                    <SelectEmployee readonly hint="This can't be edit" :clearable="false" :error-messages="(error?.data.errors?.employee_id || [''])[0]" density="compact" v-model="form.employee_id"></SelectEmployee>
...
const props = defineProps<{leave_application: LeaveRequest}>()
    ...
    onResponse(event) {
        ...
        if (event.response.ok) {
            $notification.addNotification(
                'Success',
                'Leave Application has been updated',
                'success'
            )
