    ...
    <div>
        <v-dialog width="1400" v-model="dialog" persistent>
            <template #activator="{props}">
                <v-btn v-bind="props" rounded size="small" variant="flat" color="primary" icon="mdi-pencil"></v-btn>
            </template>
                ...
                <v-card class="pa-5 rounded-lg" :disabled="status == 'pending'">
                    ...
                    <v-card-text class="px-4" style="max-height: 1000px;overflow-y: auto;">
                        <v-container>
                            ...
                            <v-row class="mb-0 pb-0">
                                <v-col class="pa-0" cols="2">
                                    <label for="reasons" class="font-weight-bold mt-3">Reason</label>
                                </v-col>
                                <v-col class="pa-0" cols="6">
                                    <v-textarea id="reasons" color="primary" variant="outlined" single-line label="Reason"
...
const props = defineProps<{change_schedule_application: ChangeScheduleApplication}>()
const $notification = useNotificationStore();
const dialog = ref(false)
const { form, displayTableId, selectedDay, addScheduleRow, pasteRow, refreshRow, removeScheduleRow } = useForm(props.change_schedule_application)
const emits = defineEmits(['update'])

const { execute, error, status } = await useApiFetch("/online-application/change-schedule-applications/" + props.change_schedule_application.id, {
    method: 'PUT',
    body: form,
    onResponse(event) {
        if (event.response.ok) {
            $notification.addNotification(
                'Success',
                'Change Schedule Application has been updated',
                'success'
            )
            emits('update')
            dialog.value = false;
