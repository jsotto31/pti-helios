...
const { data: ob_application, status, execute } = await useApiFetch("/online-application/ob-applications", {
  method: 'GET',
  immediate: true,
...
<template>
  <div class="px-5 py-3">
    <v-row>
      <v-col cols="12">
        <v-card flat>
         <PageHeader title="OB Application" subtitle="Manage OB/Wfh applications for employees.">
            <template #create>
            </template>
          </PageHeader>
                   ...
                   <v-data-table-server 
                    ...
                    <template #item.action="{ item }">
                      <div class="d-flex" style="gap: 5px;">
                        <EditDialog @update="execute" :ob_application="(item as ObRequest)"></EditDialog>
                        <DeleteDialog @delete="execute" :id="(item as ObRequest).id"></DeleteDialog>
                      </div>
                    </template>
                  </v-data-table-server>
                </v-col>
